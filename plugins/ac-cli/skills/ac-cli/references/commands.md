...
Lists live Stripe subscriptions cross-referenced with local rows. Each entry carries `plan_name` (resolved from the catalogue), `interval`, `current_period_start` / `current_period_end` (the next bill date), `cancel_at_period_end`, `created`, and `total_paid` (lifetime, minor units), plus `linked_local_subscription_id` (or `is_orphan: true` when no local row references it) and a `broken_links` list of local rows whose `stripe_subscription_id` no longer exists in Stripe. The Stripe list is paginated via `--limit` / `--offset`; `total_paid` is computed only for the returned page and the `broken_links` set is always complete. Use it to find the orphaned Stripe subscription id to pass to `ac admin subscriptions link`.
